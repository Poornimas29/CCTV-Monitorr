# scratch/scan_local_external.py
import socket
import threading

target_ip = "192.168.1.252"
found = []
lock = threading.Lock()
...
print(f"Scanning local IP {target_ip} for open ports in range 30000-50000...", flush=True)
threads = []
for port in range(30000, 50000):
    ...
    # Batch joining to prevent socket/thread exhaustion
    if len(threads) >= 1000:
        for th in threads:
            th.join()
        threads = []
